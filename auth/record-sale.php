...
if(isset($_COOKIE[$ap_tracking])){
	...
	if(isset($affiliate_balance)){
		...
		$stmt = $mysqli->prepare("INSERT INTO ap_earnings (affiliate_id, product, comission, sale_amount, net_earnings, recurring, recurring_fee, country, datetime) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
		$stmt->bind_param('sssssssss', $affiliate_id, $product, $comission, $sale_amount, $net_earnings, $recurring_period, $recurring_fee, $country, $datetime);
		$stmt->execute();
