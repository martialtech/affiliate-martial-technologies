	...
	$query = $mysqli->real_escape_string($query);
		if($result = $mysqli->query($query)){
			...
			while($row = $result->fetch_array())
				...
				//ANNUALLY RECURRING
				if($row['recurring']=='annually'){
					if($row['last_reoccurance']=='0000-00-00 00:00:00'){
						$recurring_date = date('Y-m-d', strtotime($row['datetime'] . ' +1 year'));
					}else{
						$recurring_date = date('Y-m-d', strtotime($row['last_reoccurance'] . ' +1 year'));	
					}
				}
				//SEMI-ANNUALLY RECURRING
				if($row['recurring']=='semiannually'){
					if($row['last_reoccurance']=='0000-00-00 00:00:00'){
						$recurring_date = date('Y-m-d', strtotime($row['datetime'] . ' +6 months'));
					}else{
						$recurring_date = date('Y-m-d', strtotime($row['last_reoccurance'] . ' +6 months'));	
					}
				}
				//QUARTERLY RECURRING
				if($row['recurring']=='quarterly'){
					if($row['last_reoccurance']=='0000-00-00 00:00:00'){
						$recurring_date = date('Y-m-d', strtotime($row['datetime'] . ' +3 months'));
					}else{
						$recurring_date = date('Y-m-d', strtotime($row['last_reoccurance'] . ' +3 months'));	
					}
				}
				//MONTHLY RECURRING
