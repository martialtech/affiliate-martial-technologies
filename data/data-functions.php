...
function balance($owner){
  ...
  //$currency_symbol = $money_format->getSymbol(\NumberFormatter::INTL_CURRENCY_SYMBOL); 
	...
	//echo $money_format->formatCurrency($balance); 
}
...
/* ===========================================
	DASHBOARD FUNCTIONS
   ========================================= */
function top_country_ts_table($owner){
	include('auth/db-connect.php');
	$i = 0;
	if($owner!=''){$show = 'WHERE affiliate_id='.$owner;}
	$query = "SELECT *, count(*) as total_visits FROM ap_referral_traffic $show GROUP BY country ORDER BY total_visits DESC LIMIT 0,6";
	$query = $mysqli->real_escape_string($query);
		if($result = $mysqli->query($query)){
			...
			while($row = $result->fetch_array())
				{
				$country = $row['country'];
				$get_data = mysqli_fetch_assoc(mysqli_query($mysqli, "SELECT SUM(sale_amount) as total_sales FROM ap_earnings WHERE country='".$country."'"));
				$total_sales = $get_data['total_sales'];
				//MULTI CURRENCY
				$money_format = new \NumberFormatter($locale, \NumberFormatter::CURRENCY); 
 				$currency_symbol = $money_format->getSymbol(\NumberFormatter::INTL_CURRENCY_SYMBOL); 
				$avg = $total_sales / $row['total_visits'];
				if($country!=''){
				echo '<li>
								<div class="flag flag-'.strtolower($country).'"/></div> 
									'.$country.' 
									<span class="pull-right">'.$row['total_visits'].' / '.$money_format->formatCurrency($total_sales, $currency_symbol).'</span><br>
									<div class="ss-text">'.$money_format->formatCurrency($avg, $currency_symbol).' avg per referral</div>
							</li>';
				}
			$i++;
			}
		}else{
			echo '<h3 style="text-align:center;">No data to display here yet</h3>';
		}
}

function total_device($device, $owner){
	include('auth/db-connect.php');
	if($owner!=''){$show = 'AND affiliate_id='.$owner;}
	$get_data = mysqli_fetch_assoc(mysqli_query($mysqli, "SELECT count(id) as total_device FROM ap_referral_traffic WHERE device_type=$device $show"));
	$total_device = $get_data['total_device'];
	if($total_device == ''){$total_device = '0';}
	echo $total_device;
}
		...
		if($result = $mysqli->query($query)){
			...
			while($row = $result->fetch_array())
				...
				echo '<tr>
						...
						<td>'.$row['ip'].'</td>
						<td>'; if($row['browser']==''){echo $row['agent'];}else{echo $row['browser'];} echo'</td>
						<td>'.$row['os'].'</td>
						<td><img src="" class="flag flag-'.strtolower($row['country']).'"/> '.$row['country'].'</td>
						<td>'; echo str_replace('http://'.$_SERVER['SERVER_NAME'], "", $row['landing_page']); echo '</td>';
						if($cpc_on=='1'){
							echo '<td>';
...
function banner_table($owner, $domain_path, $main_url, $admin_user){
	...
	$query = $mysqli->real_escape_string($query);
		if($result = $mysqli->query($query)){
			...
			while($row = $result->fetch_array())
				{
				if($row['url']==''){$banner_link = $main_url;}else{$banner_link = $row['url']; $banner_link = str_replace("ID", $owner, $banner_link);}
				$filetype = $row['filetype'];
				echo '<tr class="spaceUnder" style="margin:20px; border-top:1px solid #ccc;">
						<td>';
							if($filetype=='png' || $filetype=='jpg' || $filetype=='jpeg' || $filetype=='gif'){
								echo '<img src="data/banners/'.$row['filename'].'" class="banners">';
							}elseif($filetype=='video'){
								echo $row['filename'];
							}else{
								if($filetype=='pdf'){
									echo '<div style="font-size:28px;"><i class="fa fa-file-pdf" style="color:#CC0906;"></i> '.$row['filename'].'</div>';
								}elseif($filetype=='xls'){
									echo '<div style="font-size:28px;"><i class="fa fa-file-excel" style="color:#108F40;"></i> '.$row['filename'].'</div>';
								}elseif($filetype=='doc' || $filetype=='docx'){
									echo '<div style="font-size:28px;"><i class="fa fa-doc"></i> '.$row['filename'].'</div>';
								}else{
										echo '<div style="font-size:28px;"><i class="fa fa-link-ext-alt"></i> Marketing Link </div>';
								}
							}
						echo '</td>
						<td>';
						if($filetype=='png' || $filetype=='jpg' || $filetype=='jpeg' || $filetype=='gif'){ echo '
							<textarea class="banner-code"><a href="'.$banner_link.'"><img src="'.$main_url.'/data/banners/'.$row['filename'].'"></a></textarea>';
						}
						elseif($filetype=='video'){ echo '<textarea class="banner-code">'.$row['filename'].'</textarea>';
						}elseif($filetype=='customlink'){
							$display_link = str_replace("ID", $owner, $row['url']);
							echo '<textarea class="banner-code">'.$display_link.'</textarea>';
						}else{
							echo '<textarea class="banner-code">'.$main_url.'/data/banners/'.$row['filename'].'</textarea>';
						}
						echo '</td>
						<td>'; if($admin_user=='1' || $admin_user=='4'){ echo '
...
function total_device_type($device_type){
	include('auth/db-connect.php');
	$get_tdt = mysqli_fetch_assoc(mysqli_query($mysqli, "SELECT COUNT(id) as tdt FROM ap_referral_traffic WHERE device_type=$device_type"));
	$tdt = $get_tdt['tdt'];
	if($tdt==''){$tdt='0';}
	echo $tdt;
}

function get_recent_dates($owner){
	include('auth/db-connect.php');
	$i = 0;
	if($owner!=''){$show = 'WHERE affiliate_id='.$owner;}
	$query = "SELECT COUNT(id) as foo, DATE(datetime), DATE(datetime) as daterow FROM ap_referral_traffic $show GROUP BY DATE(ap_referral_traffic.datetime) ORDER BY datetime DESC";
	$query = $mysqli->real_escape_string($query);
		if($result = $mysqli->query($query)){
			$num_results = mysqli_num_rows($result);
			while($row = $result->fetch_array())
				{
				if($i<7){
					$data_dates[] = '"'.date("m/d",strtotime($row['daterow'])).'", ';
				}
				$i++;
			}
		}
	$reverse_dates = array_reverse($data_dates);
	echo implode(' ', $reverse_dates);
}

function get_recent_months($owner){
	include('auth/db-connect.php');
	$i = 0;
	if($owner!=''){$show = 'WHERE affiliate_id='.$owner;}
	$query = "SELECT COUNT(id) as foo, DATE(datetime), DATE(datetime) as daterow FROM ap_referral_traffic $show GROUP BY YEAR(ap_referral_traffic.datetime), MONTH(ap_referral_traffic.datetime) ORDER BY datetime DESC";
	$query = $mysqli->real_escape_string($query);
		if($result = $mysqli->query($query)){
			$num_results = mysqli_num_rows($result);
			while($row = $result->fetch_array())
				{
				if($i<7){
					$data_dates[] = '"'.date("F",strtotime($row['daterow'])).'", ';
				}
				$i++;
			}
		}
	return $reverse_dates = array_reverse($data_dates);
}

function get_recent_months_search($owner){
	include('auth/db-connect.php');
	$i = 0;
	if($owner!=''){$show = 'WHERE affiliate_id='.$owner;}
	$query = "SELECT COUNT(id) as foo, DATE(datetime), DATE(datetime) as daterow FROM ap_referral_traffic $show GROUP BY YEAR(ap_referral_traffic.datetime), MONTH(ap_referral_traffic.datetime) ORDER BY datetime DESC";
	$query = $mysqli->real_escape_string($query);
		if($result = $mysqli->query($query)){
			...
			while($row = $result->fetch_array())
				{
				$final_result = $num_results -1;
				if($final_result == $i && $i < 7 || $i == 0){
					$data_dates[] = $row['daterow'];
				}
				$i++;
			}
		}
...
function bar_chart_unqiue_visits($owner){
	include('auth/db-connect.php');
	if($owner!=''){$show = 'WHERE affiliate_id='.$owner;}
	$query = "SELECT COUNT(id) as uv, DATE(datetime), DATE(datetime) as daterow FROM ap_referral_traffic $show GROUP BY DATE(ap_referral_traffic.datetime) ORDER BY datetime DESC LIMIT 7";
	$query = $mysqli->real_escape_string($query);
		if($result = $mysqli->query($query)){
			$num_results = mysqli_num_rows($result);
			while($row = $result->fetch_array())
		...
		$reverse_data = array_reverse($data_rows);
		echo implode(', ', $reverse_data);
		}

function bar_chart_sales(){
	include('auth/db-connect.php');
	$query = "SELECT COUNT(id) as sales, DATE(datetime), DATE(datetime) as daterow FROM ap_earnings GROUP BY DATE(ap_earnings.datetime) ORDER BY datetime DESC";
	$query = $mysqli->real_escape_string($query);
		if($result = $mysqli->query($query)){
			$num_results = mysqli_num_rows($result);
			while($row = $result->fetch_array())
...
function bar_chart_page_views(){
	include('auth/db-connect.php');
	$query = "SELECT COUNT(id) as uv, DATE(datetime), DATE(datetime) as daterow FROM ap_referral_traffic GROUP BY DATE(ap_referral_traffic.datetime) ORDER BY datetime DESC";
	$query = $mysqli->real_escape_string($query);
		if($result = $mysqli->query($query)){
			$num_results = mysqli_num_rows($result);
			while($row = $result->fetch_array())
...
function line_chart_unqiue_visits(){
	include('auth/db-connect.php');

	$query = "SELECT COUNT(id) as uv, DATE(datetime), DATE(datetime) as daterow FROM ap_referral_traffic GROUP BY YEAR(ap_referral_traffic.datetime), MONTH(ap_referral_traffic.datetime) ORDER BY datetime DESC";
	$query = $mysqli->real_escape_string($query);
		if($result = $mysqli->query($query)){
			$num_results = mysqli_num_rows($result);
			while($row = $result->fetch_array())
...
function line_chart($admin_user, $owner){
include('auth/db-connect.php');
if($admin_user!='1'){$show = 'WHERE affiliate_id='.$owner; $show_sales = 'AND affiliate_id='.$owner; $affiliate_specific = '1';}
if($affiliate_specific == '1'){$labels = get_recent_months($owner);}else{$labels = get_recent_months();}
$labels = implode(' ', $labels);
if($affiliate_specific == '1'){$se_dates = get_recent_months_search($owner);}else{$se_dates = get_recent_months_search();}
	
	$query = "SELECT COUNT(id) as uv, DATE(datetime), DATE(datetime) as daterow FROM ap_referral_traffic $show GROUP BY YEAR(ap_referral_traffic.datetime), MONTH(ap_referral_traffic.datetime) ORDER BY datetime ASC";
	$query = $mysqli->real_escape_string($query);
		if($result = $mysqli->query($query)){
			$num_results = mysqli_num_rows($result);
			while($row = $result->fetch_array())
				...
				$get_monthly_sales = mysqli_fetch_assoc(mysqli_query($mysqli, "SELECT COUNT(*) as sales FROM ap_earnings WHERE MONTH(datetime)= '".$month."' $show_sales"));
				$sales[] = '"'.$get_monthly_sales['sales'].'"';
			}
		}
					...
					datasets: [
							{
									...
									strokeColor: "#0A5AA1",
									pointColor: "#fff",
									pointStrokeColor: "#0A5AA1",
									pointHighlightFill: "#0A5AA1",
									pointHighlightStroke: "#0A5AA1",
							...
							},
							{
									...
									strokeColor: "#FF4D00",
									pointColor: "#FFF",
									pointStrokeColor: "#FF4D00",
									pointHighlightFill: "#FF4D00",
									pointHighlightStroke: "#FF4D00",				
									data: ['.implode(', ', $sales).']
							}    		
					]';
			}
...
/* ===========================================
	PRODUCTS FUNCTIONS
   ========================================= */
function products_table($main_url, $owner){
	include('auth/db-connect.php');
	$query = "SELECT * FROM ap_products ORDER BY id DESC";
	$query = $mysqli->real_escape_string($query);
		if($result = $mysqli->query($query)){
			...
			//print_r($row);die;
				...
				$currency_symbol = $money_format->getSymbol(\NumberFormatter::INTL_CURRENCY_SYMBOL);
				echo '<tr>
						<td>';if($row['product_img']!=''){ echo '<img src="data/products/'.$row['product_img'].'" class="products">';} echo '</td>
						<td>'.$row['name'].'</td>
						<td>'.$money_format->formatCurrency($row['price'], $currency_symbol).'</td>
						<td>'; if($row['commission']=='0'){echo 'Default';}else{echo $row['commission'].'%';} echo '</td>
						<td>'; if($row['recurring']==''){echo 'Non-Recurring';}else{echo $row['recurring'];} echo '</td>
						<td>'; if($row['recurring']==''){echo 'None';}else{echo $row['recurring_fee'].'%';} echo '</td>';
						if($owner==''){ echo '<td><a class="open-integration btn btn-warning" data-id="'.$row['id'].'" data-toggle="modal" data-target="#openmodal">View Code</a></td>
						<td>			   
							<form method="post" action="data/delete-product" class="pull-left">
								<input type="hidden" name="m" value="'.$row['id'].'">
								<input type="submit" class="btn btn-sm btn-danger" value="Delete">
							</form>
						</td>';}else{
							echo '<td><textarea style="width:350px;"><a href="'.$main_url.'?ref='.$owner.'"><img src="'.$main_url.'/data/products/'.$row['product_img'].'"></a></textarea></td>';
						} echo '
					</tr>';
			}
		}
...
function site_logo($width){
	include('auth/db-connect.php');
	$get_logo = mysqli_fetch_assoc(mysqli_query($mysqli, "SELECT site_logo FROM ap_settings"));
	$site_logo = $get_logo['site_logo'];
	if($site_logo==''){
		echo '<img src="assets/img/aplogo2.png" style="width:'.$width.'" class="img-responsive">';
	}else{
		echo '<img src="data/logos/'.$site_logo.'" style="width:'.$width.'">';
	}
}
